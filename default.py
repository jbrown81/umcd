...
def upload_batch():
    form = SQLFORM.factory(
        ...
        Field('MR_field_strength', requires=IS_IN_SET(['1.5T', '3T', '4T', '7T', 'Other'])),
        Field('MR_TR'),
        Field('MR_TE'),
        Field('MR_voxel_size'),
        Field('MR_field_of_view'),
        Field('fMRI_motion_correction', 'boolean'),
        Field('fMRI_skull_stripping', 'boolean'),
        Field('fMRI_temporal_filtering', 'boolean'),
        Field('fMRI_spatial_smoothing', 'boolean'),
        Field('fMRI_slice_timing_correction', 'boolean'),
        Field('fMRI_intensity_normalization', 'boolean'),
        Field('fMRI_EPI_unwarping', 'boolean'),
        Field('fMRI_CSF_signal_regression', 'boolean'),
        Field('fMRI_white_matter_signal_regression', 'boolean'),
        Field('fMRI_global_signal_regression', 'boolean'),
        Field('DWMRI_num_directions'),
        Field('DWMRI_max_b_value'),
        Field('DWMRI_eddy_correction', 'boolean'),
        Field('DWMRI_skull_stripping', 'boolean'),
        Field('DWMRI_deterministic_tractography', 'boolean'),
        Field('DWMRI_probabilistic_tractography', 'boolean'),
        Field('sMRI_skull_stripping', 'boolean'),
        Field('sMRI_intensity_normalization', 'boolean'))
        ...
        for i in range(num_networks):
            ...
            try:
                id = db.upload_data.insert(study_name=form.vars.study_name,
						   ...
						   MR_field_strength=form.vars.MR_field_strength,
						   MR_TR=form.vars.MR_TR,
						   MR_TE=form.vars.MR_TE,
						   MR_voxel_size=form.vars.MR_voxel_size,
						   MR_field_of_view=form.vars.MR_field_of_view,
						   fMRI_motion_correction=form.vars.fMRI_motion_correction,
						   fMRI_skull_stripping=form.vars.fMRI_skull_stripping,
						   fMRI_temporal_filtering=form.vars.fMRI_temporal_filtering,
						   fMRI_spatial_smoothing=form.vars.fMRI_spatial_smoothing,
						   fMRI_slice_timing_correction=form.vars.fMRI_slice_timing_correction,
						   fMRI_intensity_normalization=form.vars.fMRI_intensity_normalization,
						   fMRI_EPI_unwarping=form.vars.fMRI_EPI_unwarping,
						   fMRI_CSF_signal_regression=form.vars.fMRI_CSF_signal_regression,
						   fMRI_white_matter_signal_regression=form.vars.fMRI_white_matter_signal_regression,
						   fMRI_global_signal_regression=form.vars.fMRI_global_signal_regression,
						   DWMRI_num_directions=form.vars.DWMRI_num_directions,
						   DWMRI_max_b_value=form.vars.DWMRI_max_b_value,
						   DWMRI_eddy_correction=form.vars.DWMRI_eddy_correction,
						   DWMRI_skull_stripping=form.vars.DWMRI_skull_stripping,
						   DWMRI_deterministic_tractography=form.vars.DWMRI_deterministic_tractography,
						   DWMRI_probabilistic_tractography=form.vars.DWMRI_probabilistic_tractography,
						   sMRI_skull_stripping=form.vars.sMRI_skull_stripping,
						   sMRI_intensity_normalization=form.vars.sMRI_intensity_normalization)
                db.analysis_results.insert(network_name=id)
            except:
...
def get_study_data():
    if len(request.args) > 0:
        ...
        for row in study:
            ...
            ages.append(row.age_range_max)
            genders.append(row.gender)
            subject_labels.append(row.subject_pool)
            group_sizes.append(row.group_size)
            
            network_name = row.network_name
            download_row = db(db.analysis_results.network_name==row.id).select().first()
            db(db.analysis_results.network_name==row.id).update(num_times_downloaded=download_row.num_times_downloaded+1)
            now = datetime.datetime.now()
            db(db.analysis_results.network_name==row.id).update(last_time_downloaded=now.strftime("%Y-%m-%d %H:%M"))
            
        with open('%s/ages_list.txt' %(new_dir), 'w') as ages_file:
            for item in ages:
                ages_file.write("%s\n" % item)
        with open('%s/genders_list.txt' %(new_dir), 'w') as genders_file:
            for item in genders:
                genders_file.write("%s\n" % item)
        with open('%s/subject_labels_list.txt' %(new_dir), 'w') as subject_labels_file:
            for item in subject_labels:
                subject_labels_file.write("%s\n" % item)
        with open('%s/group_sizes_list.txt' %(new_dir), 'w') as group_sizes_file:
            for item in group_sizes:
                group_sizes_file.write("%s\n" % item)
